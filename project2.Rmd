...
# Extracting weekly daily for further analysis
```{r}
names(dfw) <- c('Date', 'weekly_ret')
dfw= data.table(dfw, key=c('Date'))

# set weekly lead
for (i in 1:7){
    dfw <-dfw[, paste('weekly_ret_lead',paste(i),sep=""):=shift(weekly_ret, n=i, fill = NA, type='lead')]
}
dfw
```


# Calculate daily and weekly momentum for different currencies
```{r}
...
for (i in 1:4){
   ...
   for (j in 1:5){
     ...
     as.numeric(sum[[j]]$coefficients[2,'Value'], na.rm=TRUE)
...
stargazer(model_results3, type='text', header=F,
            title=paste(currencies2[j],'Bitcoin Weekly Return Twitter (', paste(min(dft$Date)),'-',paste(max(dft$Date)),')'),
            omit.stat = c("LL","ser","f"),
...
for (i in 1:5){
  ...
  stargazer(single_bin, type='text', summary=FALSE, title=paste('Bitcoin Google Search by Groups - bin',i))
}
```
...
for (i in 1:5){
  ...
  stargazer(single_bin, type='text', summary=FALSE, title=paste('Bitcoin Google Search by Groups (since 2013) - bin',i))
}
```
...
for (i in 1:5){
  ...
  stargazer(single_bin, type='text', summary=FALSE, title=paste('Bitcoin Hack by Groups - bin',i))
}
